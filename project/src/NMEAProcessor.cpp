...
bool ParseNMEAMessage(const QString & strMsg, NMEAMessage * pMsg)
{
	...
	switch (eType) {
		...
		case NMEAMessageTypeGPGLL:
			...
			pMsg->sGPGLL.iValid = (*(++iterArg))[0].toAscii();
			break;
		case NMEAMessageTypeGPGSA:
			pMsg->sGPGSA.iAcquisitionMode = (*(++iterArg))[0].toAscii();
		...
		case NMEAMessageTypeGPRMC:
			...
			pMsg->sGPRMC.iValid = (*(++iterArg))[0].toAscii();
			...
			pMsg->sGPRMC.iVariationSense = (*(++iterArg))[0].toAscii();
			break;
